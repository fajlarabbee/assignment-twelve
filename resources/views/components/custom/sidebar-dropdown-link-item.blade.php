@props(['name' => '', 'label' => 'Dropdown Label', 'links' => array()])

<li class="menu nav-item">
    <button
        type="button"
        class="nav-link group"
        :class="{'active' : activeDropdown === '{{ $name }}'}"
        @click="activeDropdown === '{{ $name }}' ? activeDropdown = null : activeDropdown = '{{ $name }}'"
    >
        ...
        <div class="rtl:rotate-180" :class="{'!rotate-90' : activeDropdown === '{{ $name }}'}">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M9 5L15 12L9 19" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
        </div>
    </button>
    <ul x-cloak x-show="activeDropdown === '{{ $name }}'" x-collapse class="sub-menu text-gray-500">
